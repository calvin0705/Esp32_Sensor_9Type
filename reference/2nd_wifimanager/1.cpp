...
WiFiManagerParameter custom_field_1; // global param ( for non blocking w params )
WiFiManagerParameter custom_field_2; // global param ( for non blocking w params )
WiFiManagerParameter custom_field_3; // global param ( for non blocking w params )
...
void setup() {
  ...
  // new (&custom_field) WiFiManagerParameter("customfieldid", "mqtt_server_ip", "", customFieldLength,"placeholder=\"Custom Field Placeholder\"");
  ...
  res = wm.autoConnect("水木_1","00000000"); // password protected ap
...
void checkButton(){
  ...
  if ( digitalRead(TRIGGER_PIN) == LOW ) {
    ...
    if( digitalRead(TRIGGER_PIN) == LOW ){
      ...
      if (!wm.startConfigPortal("水木_2","00000000")) {
        Serial.println("failed to connect or hit timeout");
        delay(3000);
        // ESP.restart();
      } else {
